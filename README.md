...
**Author:** B Kasinath ([GitHub](https://github.com/kasitpm) · [LinkedIn](https://linkedin.com/in/kasinaththirupuram) · [Portfolio](https://kasitpm.github.io))
...
- **`fetch_data.py`** — a Python script that pulls rainfall and river-discharge forecasts from the [Open-Meteo](https://open-meteo.com/) API, and parses official weather/flood alerts from the [NDMA SACHET](https://sachet.ndma.gov.in/) CAP feed (which aggregates warnings from IMD, the Central Water Commission, and state disaster authorities).
...
1. Fork this repo.
2. Enable GitHub Actions (Settings → Actions → General → set to "Read and write permissions").
3. Run the `Update Kerala data` workflow once manually, or wait for its scheduled run.
4. Open `Kerala_Flood_Watch.pbix` in Power BI Desktop — the queries already point at this repo's raw CSV URLs, so update them to point at your fork if you want independent data.
